...
test('Draft project, to do column,tags Non Priority and On track', async ({ page }) => {
  await page.goto('https://app.asana.com/-/login');
  await page.getByLabel('Email address').click();
  await page.getByLabel('Email address').fill('ben+pose@workwithloop.com');
  await page.getByRole('button', { name: 'Continue', exact: true }).click();
  ...
  // await page.getByText('Non-Priority');
  // await page.getByText('On-track');
  // await expect(page.getByLabel('Cross-functional project plan', { text: true })).toContainTag('Non-Priority');

  // @ts-ignore
  await page.getByText('Non-Priority').hover();
  // await page.getByLabel('Cross-functional project plan').click();
  
  await page.getByText('On-track').first();
  // await page.getByText('On-track').hover();
});
...
test("Test Case 6", async ({ page }) => {
  await page.
  goto("https://app.asana.com/-/login");
  await page.getByLabel("Email address").click();
  await page.getByLabel("Email address").fill("ben+pose@workwithloop.com");
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await page.getByLabel("Password", { exact: true }).fill("Password123");
